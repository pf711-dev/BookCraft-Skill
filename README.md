...
BookCraft 是一个将英文 EPUB 电子书、Markdown 文件或 PDF 一键翻译成中英对照或纯中文格式的 Skill。


## 功能

- **三格式支持**：EPUB / Markdown / 文本型 PDF（扫描页占比超 60% 会提示先 OCR）
- **PDF 翻译**：结构化提取（章节/段落/图片锚点/页眉页脚剔除/跨页段落合并），
  三种章节检测策略（书签目录 / 字号自动检测 / 正则模式）；数据表行与信头地址
  保留原文防误译；整页扫描件译文在前、原图附后；输出中文 EPUB + Markdown 备份
